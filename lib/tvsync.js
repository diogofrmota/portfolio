...
export const fallbackDiscovery = [
  { id: 11224, mediaType: 'movie', title: 'Cinema Paradiso', year: '1988', overview: 'A filmmaker remembers the cinema that shaped his childhood and the projectionist who became his friend.', voteAverage: 8.4, posterPath: null, backdropPath: null },
  { id: 1399, mediaType: 'tv', title: 'Game of Thrones', year: '2011', overview: 'Noble families fight for control of the lands of Westeros while an ancient enemy returns.', voteAverage: 8.5, posterPath: null, backdropPath: null },
  { id: 157336, mediaType: 'movie', title: 'Interstellar', year: '2014', overview: 'Explorers travel through a wormhole in space in an attempt to ensure humanity’s survival.', voteAverage: 8.5, posterPath: null, backdropPath: null },
  { id: 1396, mediaType: 'tv', title: 'Breaking Bad', year: '2008', overview: 'A chemistry teacher turns to an increasingly dangerous life of crime.', voteAverage: 8.9, posterPath: null, backdropPath: null },
  { id: 550, mediaType: 'movie', title: 'Fight Club', year: '1999', overview: 'An insomniac office worker meets a charismatic soap maker and builds an underground club.', voteAverage: 8.4, posterPath: null, backdropPath: null },
  { id: 100088, mediaType: 'tv', title: 'The Last of Us', year: '2023', overview: 'A hardened survivor escorts a teenager across a transformed America.', voteAverage: 8.5, posterPath: null, backdropPath: null },
  { id: 27205, mediaType: 'movie', title: 'Inception', year: '2010', overview: 'A skilled thief enters dreams to steal secrets and is offered a chance to erase his past.', voteAverage: 8.4, posterPath: null, backdropPath: null },
  { id: 85552, mediaType: 'tv', title: 'Euphoria', year: '2019', overview: 'A group of high-school students navigate identity, trauma, friendship, and love.', voteAverage: 8.3, posterPath: null, backdropPath: null },
];
...
export async function getTvsyncDiscovery() {
  try {
    ...
    return items?.length ? items : fallbackDiscovery;
  } catch {
...
export async function searchTvsyncDiscovery(query) {
  ...
  try {
    ...
    if (!payload) return fallbackDiscovery.filter((item) => item.title.toLowerCase().includes(cleanQuery.toLowerCase()));
...
export async function getTvsyncMediaDetails(mediaType, id) {
  ...
  try {
    ...
    // Fall through to the built-in preview details when TMDB is unavailable.
  }
  const preview = fallbackDiscovery.find((item) => item.id === safeId && item.mediaType === safeType);
  return preview ? { ...preview, genres: [], runtime: null, status: '', tagline: '', trailerUrl: null, cast: [], crew: [], seasons: [] } : null;
}
